...
import com.example.firebase23041909.databinding.ActivityMainBinding
                ...
                MyApplication.firebaseAuth.signInWithCredential(credential)
                    .addOnCompleteListener(this){ task ->
                        if (task.isSuccessful){
                            ...
                            Toast.makeText(this@MainActivity, "구글 로그인에 실패하셨습니다.", Toast.LENGTH_SHORT).show()
                        }
                    }
            }catch (e: ApiException){
                //구글 로그인 인증 API 예외발생
                binding.authMainTextView.text = "구글 로그인 인증 API 예외발생"
                Toast.makeText(this@MainActivity, "구글 로그인 API 인증 실패.", Toast.LENGTH_SHORT).show()
                            ...
                            MyApplication.firebaseAuth.currentUser?.sendEmailVerification()
                                ?.addOnCompleteListener { sendTask ->
                                    if (sendTask.isSuccessful){
                                        ...
                                        //이메일, 패스워드 전송메일발송 실패 메세지
                                        Toast.makeText(this@MainActivity, "파이어베이스 전송메일 발송실패", Toast.LENGTH_SHORT).show()
                                        binding.authMainTextView.text = "파이어베이스 가입 실패"
                                        changeStateButton("처음")
                                    }
                                }
                        }else{
                            //이메일, 패스워드 인증실패 메세지
                            Toast.makeText(this@MainActivity, "파이어베이스 이메일, 패스워드 인증 실패", Toast.LENGTH_SHORT).show()
