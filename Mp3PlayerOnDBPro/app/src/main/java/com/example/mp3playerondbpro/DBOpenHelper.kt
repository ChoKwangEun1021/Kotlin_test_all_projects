...
class DBOpenHelper(context: Context, dbName: String, version: Int) :
    ...
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {
        val dropTalbe = "drop table musicTBL"
        db?.execSQL(dropTalbe)
        this.onCreate(db)
    }
    ...
    fun insertMusicTBL(musicData: MusicData) {
        val insertMusic =
            "insert into musicTBL values('${musicData.id}', '${musicData.title}', '${musicData.artist}'," +
    ...
    fun selectMusicData(id: String): MusicData? {
        ...
        val selectMusic = "select * from musicTBL where id = '${id}'"
        val db = this.readableDatabase
